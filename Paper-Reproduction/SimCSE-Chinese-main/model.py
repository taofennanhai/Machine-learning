...
class SimcseModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        if self.pooling == 'pooler':    # 这是序列的第一个token(classification token)的最后一层的隐藏状态，它是由线性层和Tanh激活函数进一步处理的
...
def simcse_unsup_loss(y_pred, device, temp=0.05):
    ...
    test = y_pred.unsqueeze(1)
    test1 = y_pred.unsqueeze(0)
...
if __name__ == '__main__':
    ...
    loss = simcse_unsup_loss(y_pred, 'cpu', temp=0.05)
    print(loss)
