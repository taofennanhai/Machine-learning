...
def load_train_data_unsup(tokenizer, args):
    ...
    获取无监督训练语料
    """
    logger.info('loading unsupervised train SecondQuestionData')
    ...
    if os.path.exists(train_file_cache) and not args.overwrite_cache:
        with open(train_file_cache, 'rb') as f:
            ...
            logger.info("len of train SecondQuestionData:{}".format(len(feature_list)))
            return feature_list
    feature_list = []
    with open(args.train_file, 'r', encoding='utf8') as f:
        ...
        for line in tqdm(lines):
            ...
            feature = tokenizer([line, line], max_length=args.max_len, truncation=True, padding='max_length',
                                return_tensors='pt')    # 这个部分已经是tensor
            test = feature['input_ids'].numpy().tolist()
            text = tokenizer.decode(test[0])
...
def load_train_data_sup(tokenizer, args):
    ...
    获取监督训练语料
    """
    logger.info('loading supervised train SecondQuestionData')
    output_path = os.path.dirname(os.path.dirname(args.output_path))
...
def load_eval_data(tokenizer, args, mode):
    ...
    logger.info('loading {} SecondQuestionData'.format(mode))
    output_path = os.path.dirname(os.path.dirname(args.output_path))
    ...
    if os.path.exists(eval_file_cache) and not args.overwrite_cache:
        with open(eval_file_cache, 'rb') as f:
            feature_list = pickle.load(f)
            logger.info("len of {} SecondQuestionData:{}".format(mode, len(feature_list)))
            return feature_list
...
def main(args):
    ...
    if args.do_train:
        ...
        print('best dev corrcoef:{}'.format(corrcoef))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_file", type=str, default="SecondQuestionData/stsb_unsup_train_data.txt")
    # parser.add_argument("--train_file", type=str, default="SecondQuestionData/cmnli_sup_train_data.csv")
    parser.add_argument("--dev_file", type=str, default="SecondQuestionData/STS-B/dev.txt")
