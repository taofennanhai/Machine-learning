...
def get_attention_pad_mask(seq_q, seq_k):
    ...
    pad_attention_mask = seq_q.data.eq(0).unsqueeze(1)  # [batch_size, 1, len_k], True is masked, eq函数判断是否为0，是就返回true
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model=512, d_v=64, d_k=64, n_heads=8, device='cpu'):
    ...
    def forward(self, input_Q, input_K, input_V, attention_mask):    # 输入三次，是为了方便在encoder和decoder复用，decoder那块需要encoder的输入
        ...
        context = ScaledDotProductAttention().forward(Q, K, V, attention_mask)    # 进行QKV矩阵相乘
        ...
        return nn.LayerNorm(self.d_model).forward(output + residual)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, encoder_inputs, encoder_self_attention_mask):
        ...
        # 第二个enc_inputs * W_K = K
        # 第三个enc_inputs * W_V = V
        encoder_outputs = self.encoder_self_attention(encoder_inputs, encoder_inputs, encoder_inputs,
                                                                 encoder_self_attention_mask)  # enc_inputs to same Q,K,V（未线性变换前）
