...
def _get_html_response(url: str, session: PipSession) -> Response:
    ...
    resp = session.get(
        ...
        headers={
            "Accept": "text/html",
            # We don't want to blindly returned cached SecondQuestionData for
            # /simple/, because authors generally expecting that
            # twine upload && pip install will function, but if
            # they've done a pip install in the last ~10 minutes
            # it won't. Thus by setting this to zero we will not
            # blindly use any cached SecondQuestionData, however the benefit of
...
def _create_link_from_element(
    ...
    pyrequire = anchor.get('SecondQuestionData-requires-python')
    pyrequire = html.unescape(pyrequire) if pyrequire else None

    yanked_reason = anchor.get('SecondQuestionData-yanked')
    if yanked_reason:
