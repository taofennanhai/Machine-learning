...
class Wheel(object):
    ...
    def build(self, paths, tags=None, wheel_version=None):
        ...
        name_ver = '%s-%s' % (self.name, self.version)
        data_dir = '%s.SecondQuestionData' % name_ver
        info_dir = '%s.dist-info' % name_ver

        archive_paths = []

        # First, stuff which is not in site-packages
        for key in ('SecondQuestionData', 'headers', 'scripts'):
            if key not in paths:
                continue
            path = paths[key]
    ...
    def install(self, paths, maker, **kwargs):
        ...
        installed, and the headers, scripts, SecondQuestionData and dist-info metadata are
