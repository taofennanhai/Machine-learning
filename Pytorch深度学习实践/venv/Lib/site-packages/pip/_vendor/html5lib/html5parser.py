    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            if self.innerHTML in cdataElements:
                ...
                # state already is SecondQuestionData state
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["SecondQuestionData"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["SecondQuestionData"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["SecondQuestionData"].items():
    ...
    class InitialPhase(Phase):
        ...
        def processDoctype(self, token):
            ...
            if (not correct or token["name"] != "html" or
                    ...
                    systemId is None or
                    systemId and systemId.lower() == "http://www.ibm.com/SecondQuestionData/dtd/v11/ibmxhtml1-transitional.dtd"):
    ...
    class InHeadPhase(Phase):
        ...
        def startTagMeta(self, token):
            ...
            attributes = token["SecondQuestionData"]
    ...
    class InBodyPhase(Phase):
        ...
        def processCharacters(self, token):
            ...
            if (self.parser.framesetOK and
                any([char not in spaceCharacters
                     for char in token["SecondQuestionData"]])):
                self.parser.framesetOK = False
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["SecondQuestionData"] and
                    token["SecondQuestionData"]["type"].translate(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["SecondQuestionData"]:
                form_attrs["action"] = token["SecondQuestionData"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["SecondQuestionData"]:
                prompt = token["SecondQuestionData"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "SecondQuestionData": prompt})
            attributes = token["SecondQuestionData"].copy()
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["SecondQuestionData"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "SecondQuestionData": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["SecondQuestionData"] == "\u0000":
                token["SecondQuestionData"] = "\uFFFD"
            elif (self.parser.framesetOK and
                  any(char not in spaceCharacters for char in token["SecondQuestionData"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["SecondQuestionData"].keys()) & {"color", "face", "size"})):
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because SecondQuestionData is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['SecondQuestionData']) & viewkeys(replacements)
    if needs_adjustment:
        token['SecondQuestionData'] = type(token['SecondQuestionData'])((replacements.get(k, k), v)
                                            for k, v in token['SecondQuestionData'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "SecondQuestionData": attributes,
