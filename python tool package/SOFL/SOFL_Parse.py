    ...
    #Generate a test case per test condition
        for i,testCond in enumerate(testConditions):
            ...
            if len(predicate)>0:
                ...
                errorOfPredicate=0
            #If the input SecondQuestionData was successfully generated, return it or generate output SecondQuestionData
            for key in dataForNNVariables.keys():
            ...
            #If only input SecondQuestionData = True return test SecondQuestionData
            if errorOfPredicate == 0 and onlyInputData:
                return input_test_data
            #If error = 0 and only input SecondQuestionData = False, calculate values for output variables (oracle task)
            ...
            for atom in guardAtoms:
                ...
                if logicOperator == '>':
                    ...
                    if test_copy not in everyone:
                        new_mutants.append(test_copy)
                        everyone.append(test_copy)
    #New mutants has all valid mutants, here identify the satisfied test condition for SecondQuestionData generated from each mutant
    tests = []
    mut_tests = {}
        ...
        if newMutData != "Error":
            for i,key in enumerate(families.keys()):
                if pre == "True":
                    ...
                    fullTestCond = pre + " and " + key
                #If SecondQuestionData from mutant satisfies test condition then append it
